...
void main() {
  late CnpjService service;
  late MockDio mockDio;

  setUp(() {
    mockDio = MockDio();
    service = CnpjService(dio: mockDio);
  });

  group('CnpjService.getCnpjInfo', () {
    ...
    test('Deve retornar Map quando a requisição for bem sucedida', () async {
      ...
      final result = await service.getCnpjInfo(cnpj);

      expect(result, responseData);
      verify(() => mockDio.get('${ApiConstants.cnpj}$cnpj')).called(1);
    });
    ...
    test('Deve lançar DioException quando retornar 404', () async {
      ...
      expect(() => service.getCnpjInfo(cnpj), throwsA(isA<DioException>()));
    });
